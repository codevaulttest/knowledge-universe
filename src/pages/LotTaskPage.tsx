...
function LotTaskCompletionDetail({ label, snapshot, lotQuota, historySnapshot, showTodaySummary = false }: { label: string; snapshot: TaskDaySnapshot | null; lotQuota: LotQuota; historySnapshot?: TaskDaySnapshot | null; showTodaySummary?: boolean }) {
  ...
  return <div className={`task-calendar-detail${showTodaySummary ? '' : ' task-calendar-detail--history'}`}>
    ...
    {showTodaySummary && <div className="task-calendar-detail-conditions"><span className={`task-calendar-detail-status${postCountDone ? ' is-posted' : ''}`}>{postCountDone ? <Check size={13} strokeWidth={2.6} /> : <Circle size={13} strokeWidth={1.9} />}{t('发帖')}<strong>{t('已发')} <span className="task-calendar-detail-number">{snapshot.postedCount}</span> / <span className="task-calendar-detail-number">{requiredPostCount}</span> {t('篇')}</strong></span><span className={`task-calendar-detail-status${interactionDone ? ' is-posted' : ''}`}>{interactionDone ? <Check size={13} strokeWidth={2.6} /> : <Circle size={13} strokeWidth={1.9} />}{t('跟别人的帖子互动')}<strong>{t('已互动')} <span className="task-calendar-detail-number">{snapshot.interactedCount}</span> / <span className="task-calendar-detail-number">{requiredInteractionCount}</span> {t('次')}</strong></span></div>}

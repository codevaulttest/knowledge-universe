...
import { AlertTriangle, Check, ChevronRight, FileText, Info, PartyPopper, Sparkles, X } from 'lucide-react';
...
export function TaskPanelSheet({ onClose }: { onClose: () => void }) {
  const { t, taskSnapshotToday, taskSnapshotYesterday, taskCelebrateSignal } = useApp();
      ...
      <div className="sheet-backdrop" onClick={onClose}>
        <div className="payment-sheet task-panel-sheet" role="dialog" aria-modal="true" onClick={e => e.stopPropagation()}>
          ...
          <div className="task-card task-card--interaction">
            ...
            {celebrate && (
              <div className="task-celebrate" role="status">
                <PartyPopper size={14} strokeWidth={2} aria-hidden="true" />
                <span>{t('已完成 {count} 篇，当前可领取比例 {ratio}%', { count: snapshot.interactedCount, ratio: snapshot.claimRatio })}</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="payment-sheet pb-info-sheet" role="dialog" aria-modal="true" onClick={e => e.stopPropagation()}>
        ...
        <div className="pb-info-sheet-body">
          ...
          <p className="pb-info-sheet-para">
            ...
            {t('系统每天推荐 {total} 篇互动帖，对任意一篇进行点赞/评论/收藏/踩任一操作即视为完成 1 篇（同一帖子多次操作只算一次）。', { total: TASK_INTERACTION_POOL_SIZE })}
          </p>
          <p className="pb-info-sheet-para">
